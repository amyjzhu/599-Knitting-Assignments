...
function do_a_tube() {
    ...
    let sts = [26, 25, 24, 22, 21, 20, 18, 17, 16, 14, 13, 12, 10, 9, 8, 6, 5, 4, 2, 1, 0]
    sts.reverse();
    ...
    k.xfer(`f${(width) * repeat_size + 1}`, `b${(width) * repeat_size + 1}`);
    ...
    for (var h = 0; h < height; h++) {
        // last three
        k.knit("-", `f${(width) * repeat_size + 2}`, carrier);
        k.knit("-", `f${(width) * repeat_size + 1}`, carrier);
        k.knit("-", `f${(width) * repeat_size}`, carrier);
    ...
    for (var s = (width / 2) + 1; s <= width; s++) { // never 0 because that's fine 
        k.xfer(`f${s * repeat_size}`, `b${s * repeat_size}`)
        k.xfer(`f${s * repeat_size + 1}`, `b${s * repeat_size + 1}`)
        k.xfer(`f${s * repeat_size + 2}`, `b${s * repeat_size + 2}`)
    }
    ...
    // put purls back
    k.xfer(`f${(width) * repeat_size + 1 - shift_back}`, `b${(width) * repeat_size + 1 - shift_back}`);
    ...
    for (var h = 0; h < height; h++) {
        // last three
        k.knit("-", `f${(width) * repeat_size + 2 - shift_back}`, carrier);
        k.knit("-", `f${(width) * repeat_size + 1 - shift_back}`, carrier);
        k.knit("-", `f${(width) * repeat_size - shift_back}`, carrier);
